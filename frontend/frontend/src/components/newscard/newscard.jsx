...
const Newscard = ({ newsvalue }) => {
  ...
  return (
    <div className="news-card d-flex flex-md-row flex-column justify-content-md-center align-items-md-center" onClick={handleClick}>
      <img src={newsvalue.image_url || newsvalue.image} alt="News" className="news-img" />
      <div className="news-content">
        <div className="news-meta">
          <img src="/assets/dateicon.svg" alt="calendar" className="meta-icon" />
          <span>{newsvalue.date}</span>
          <span className="divider">|</span>
          <img src="/assets/commenticon.svg" alt="comment" className="meta-icon" />
          <span>1 Comment ({newsvalue.comments})</span>
        </div>
        <h5 className="news-title">{newsvalue.title}</h5>
        <div className="news-admin ">
          <div className="admin-avatar" >
            <img src={newsvalue.admin_image || newsvalue.adminimage} alt="" />
          </div>
